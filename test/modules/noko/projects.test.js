...
import * as Noko from '../../../server/modules/noko/projects';
...
describe('Noko Project', () => {
  beforeEach(async () => {
    moxios.install();
    await models.Automation.destroy({ force: true, truncate: true, cascade: true });
  ...
  describe('saveNokoProject', () => {
    it('should return error messages successfully', () => {
      const { saveNokoProject } = Noko;
      const nokoError = saveNokoProject({}, 'this is an error message', 'failure');
  ...
  describe('assignProject', () => {
    it('should assign a user to a project', async () => {
      mockNokoGetCreateUser(`&email=${email}`, 200, [{ id: 5 }]);
      mockNockAssignProject(5, 200, 20000);
    ...
    it('should catch errors passed within method', async () => {
      mockNokoGetCreateUser(`&email=${email}`, 200, [{ id: 8 }]);
      mockNockAssignProject(8, 400, { message: 'esa can catch errors' });
